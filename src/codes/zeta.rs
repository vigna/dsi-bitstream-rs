...
pub trait ZetaReadParam<E: Endianness>: MinimalBinaryRead<E> {
    ...
    fn skip_zetas_param<const USE_TABLE: bool>(&mut self, k: u64, n: usize) -> Result<usize>;
    ...
    fn skip_zeta3s_param<const USE_TABLE: bool>(&mut self, n: usize) -> Result<usize>;
}
...
impl<B: BitRead<BE>> ZetaReadParam<BE> for B {
    ...
    fn skip_zetas_param<const USE_TABLE: bool>(&mut self, k: u64, n: usize) -> Result<usize> {
        ...
        for _ in 0..n {
            skipped_bits += default_skip_zetas_param(self, k)?;
        }
        Ok(skipped_bits)
    }
    ...
    fn skip_zeta3s_param<const USE_TABLE: bool>(&mut self, n: usize) -> Result<usize> {
        ...
        for _ in 0..n {
            skipped_bits += 'outer: {
                ...
                default_skip_zetas_param(self, 3)?
            }
        }
        Ok(skipped_bits)
    }
...
#[inline(always)]
fn default_skip_zetas_param<BO: Endianness, B: BitRead<BO>>(
    backend: &mut B,
    k: u64,
) -> Result<usize> {
    ...
    let u = 1 << ((h + 1) * k);
    let l = 1 << (h * k);
    let res = backend.skip_minimal_binaries(u - l, 1)?;
    Ok(h as usize + 1 + res)
}
