...
pub fn write_table_le<B: BitWrite<LE>>(backend: &mut B, n: u64) -> Result<Option<usize>, B::Error> {
    Ok(if let Some((bits, len)) = WRITE_LE.get(n as usize) {
...
pub fn write_table_be<B: BitWrite<BE>>(backend: &mut B, n: u64) -> Result<Option<usize>, B::Error> {
    Ok(if let Some((bits, len)) = WRITE_BE.get(n as usize) {
        backend.write_bits(*bits as u64, *len as usize)?;
        Some(*len as usize)
    } else {
        None
...
pub const fn check_read_table(peek_bits: usize) {
    assert!(peek_bits >= READ_BITS, "BitRead peek word too small for gamma code read tables (1 bits required)");
}
