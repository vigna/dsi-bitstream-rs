...
data = [
    dict(zip(header, [x.strip() for x in line.split("\t")]))
    for line in data[1:]
]
...
def create_plot(operations, title):
    ...
    little_endian = [(d['median'], d['25%'], d['75%']) 
                     for code in codes 
                     for d in operations 
    ...
    yerr_le = np.array([np.array(medians_le) - np.array(q25_le), 
                        np.array(q75_le) - np.array(medians_le)])
    
    # Plot big endian data
    big_endian = [(d['median'], d['25%'], d['75%']) 
                  for code in codes 
                  for d in operations 
    ...
    yerr_be = np.array([np.array(medians_be) - np.array(q25_be), 
    ...
    for i, (median_le, median_be) in enumerate(zip(medians_le, medians_be)):
        ...
        ax.text(i - width/2, median_le + (q75_le[i] - q25_le[i])/2 + 0.1,
                f'{median_le:.3f}', rotation=90, ha='center', va='bottom')
        # Add label for big endian
        ax.text(i + width/2, median_be + (q75_be[i] - q25_be[i])/2 + 0.1,
                f'{median_be:.3f}', rotation=90, ha='center', va='bottom')
