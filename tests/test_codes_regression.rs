...
macro_rules! test_code {
    ($call_be:expr, $expected_be:expr, $call_le:expr, $expected_le:expr,) => {
        ...
        let mut writer = <BufBitWriter<BE, u128, _>>::new(MemWordWriterVec::new(&mut data));
        $call_be(&mut writer)?;
        drop(writer);
        assert_eq!(
            data[0].to_be(),
        ...
        let mut data = vec![0_u64];
        let mut writer = <BufBitWriter<LE, u128, _>>::new(MemWordWriterVec::new(&mut data));
        $call_le(&mut writer)?;
        drop(writer);
        assert_eq!(
...
type Backend<'a, E> = BufBitWriter<E, u128, MemWordWriterVec<u64, &'a mut Vec<u64>>>;
