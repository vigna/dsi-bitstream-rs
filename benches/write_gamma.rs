...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("write_gamma<BE> (no tables)", |b| {
        b.iter(|| {
            black_box(w_be.write_gamma_param::<false>(s[i % (1 << 24)]).unwrap());
            i += 1;
        })
    });
    ...
    c.bench_function("write_gamma<LE> (no tables)", |b| {
        b.iter(|| {
            black_box(w_le.write_gamma_param::<false>(s[i % (1 << 24)]).unwrap());
            i += 1;
        })
    });
    ...
    c.bench_function("write_gamma<BE> (tables)", |b| {
        b.iter(|| {
            black_box(w_be.write_gamma_param::<true>(s[i % (1 << 24)]).unwrap());
            i += 1;
        })
    });
    ...
    c.bench_function("write_gamma<LE> (tables)", |b| {
        b.iter(|| {
            black_box(w_le.write_gamma_param::<true>(s[i % (1 << 24)]).unwrap());
            i += 1;
        })
    });
}
